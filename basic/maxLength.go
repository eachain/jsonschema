...
func MaxLength(ctx *schema.Context, js jsi.JSON) (schema.Validator, *schema.Result) {
...
func ValidateMaxLength(cmp schema.Compiler) schema.CompileFunc {
...
func (m *MaxLengthValidator) Validate(ctx *schema.Context, js jsi.JSON) *schema.Result {
	...
	if utf8.RuneCountInString(val) > m.Value {
		return schema.WithError(schema.Error{
			...
			Type:  js.Type(),
			Value: js,
			Msg:   "string length should be lte " + m.Number,
		})
	}
	return nil
}
